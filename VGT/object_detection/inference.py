...
def main():
    ...
    for i in range(len(output)):
        bbox = output.pred_boxes.tensor[i].tolist()  # Coordonnées du rectangle détecté
        score = float(output.scores[i])  # Probabilité de la classe
        label = md.thing_classes[output.pred_classes[i]]  # Nom de la classe
    ...
    json_output_path = os.path.join(args.output_root, args.image_name + "_results.json")

    # Sauvegarder les résultats en JSON
    with open(json_output_path, "w") as f:
        json.dump(results_list, f, indent=4)

    print(f"Résultats d'inférence enregistrés dans {json_output_path}")
